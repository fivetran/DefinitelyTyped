...
declare module CodeMirror {
    ...
    function defineMode<State>(name: string, modeProvider: (editorConfig: EditorConfiguration, modeConfig?: ModeConfiguration) => ModeObject<State>): void;
    ...
    interface EditorChangeCancellable extends CodeMirror.EditorChange {
        /**
         * may be used to modify the change. All three arguments to update are optional, and can be left off to
         * leave the existing value for that field intact.
         */
    ...
    interface ModeObject<State> {
         ...
         * style string, or null for tokens that do not have to be styled
         */
        token(stream: Stream, state: State): string;

        /**
         * By default, blank lines are simply skipped when tokenizing a document.
         ...
         * so that it can restart a parse at any line, copies must be made of state objects.
         * The default algorithm used is that a new state object is created, which gets all the properties of the old object.
         * Any properties which hold arrays get a copy of these arrays (since arrays tend to be used as mutable stacks).
        ...
        /**
         * string that starts a block comment, to work with commenting addon
         */
        blockCommentStart?: string;

        /**
         * string that ends a block comment, to work with commenting addon
         */
        blockCommentEnd?: string;

        /**
         * trigger a reindent whenever one of the characters in this string are typed
         */
        electricChars?: string;

        /**
         * trigger indentation when the part of the line before the cursor matches the expression
         */
    ...
    interface Stream {
        ...
        /**
         * Skips to the next occurrence of the given character, if found on the current line
        ...
        /**
         * Act like a multi-character eat—if consume is true or not given—or a look-ahead that doesn't update
         * the stream position—if it is false. pattern can be either a string or a regular expression starting with ^.
         * When it is a string, caseFold can be set to true to make the match case-insensitive.
         * When successfully matching a regular expression, the returned value will be the array returned by match,
        ...
        /**
         * Backs up the stream n characters. Backing it up further than the start of the current token
         * will cause things to break, so be careful.
         */
